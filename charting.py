...
def plot_full_analysis(
    ...
    if "BB_Upper" in df.columns:
        ax_price.plot(np.arange(len(df)), df["BB_Upper"].values, color="#9e9e9e",
                      linestyle="--", linewidth=0.8, alpha=0.7, label="BB Upper")
        ax_price.plot(np.arange(len(df)), df["BB_Lower"].values, color="#9e9e9e",
                      linestyle="--", linewidth=0.8, alpha=0.7, label="BB Lower")

    # Support/resistance levels
    levels = (snap or {}).get("levels", {}) or {}
    for label, val, style in [
        ("Resistance", levels.get("nearest_resistance"), {"color": "#d32f2f", "linestyle": "--"}),
        ("Next R",     levels.get("next_resistance"),    {"color": "#d32f2f", "linestyle": ":"}),
